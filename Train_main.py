...
- 2026-06-30；作者：GG。清理面向使用者的历史兼容表述，说明聚焦当前公开数据库结构。
English: - 2026-06-30; Author: GG. clean user-facing compatibility wording and focus on the current public database structure.
"""
...
# 调参影响 / Tuning effect:
# EN: / Tuning effect:
# - RESUME_TRAINING=True：继续指定目录中未完成的模型/Fold，适合断点恢复；目录不匹配会导致实验混写或直接报错。
# EN: RESUME_TRAINING=True: continue unfinished models/Folds in the specified directory; mismatched directories can mix experiments or raise errors.
# - RESUME_TRAINING=False：创建新的输出目录，适合正式新实验；不会复用已有训练进度。
...
# - SPLIT_SEED 改变：会产生新的折分，不能再与既有 Full/shared_folds 直接一一对比。
...
LR_PATIENCE_SMALL_BATCH_THRESHOLD = 8  # 小 batch patience 阈值保留字段。 / EN: retained small-batch patience threshold.
LR_PATIENCE_SMALL_BATCH_MULTIPLIER = 2  # 小 batch patience 倍率保留字段。 / EN: retained small-batch patience multiplier.
LR_PATIENCE_BATCH_ONE_MULTIPLIER = 8  # batch=1 patience 倍率保留字段。 / EN: retained batch-size-one patience multiplier.
...
# 用法示范 / Examples:
# EN: usage examples / Examples:
# - 当字段没有在上方面板中显式列出，但训练 Config 中确实存在时，可用 ATTR=VALUE 临时覆盖。
# EN: if a field is not listed above but exists in the training Config, use ATTR=VALUE as a temporary override.
